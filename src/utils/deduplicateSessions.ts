 ...
 * When two sessions share the same game/formula, type, track, and lap count
 * and are within 30 s of each other, keep the larger file (more complete telemetry).
...
export function deduplicateSessions<T extends WithFileSize>(sessions: T[]): T[] {
  ...
  for (const s of sessions) {
    const key = `${s.gameYear ?? ""}|${s.packetFormat ?? ""}|${s.formula ?? ""}|${s.sessionType}|${s.track}|${s.validLapCount}`;
