...
const DETAIL_VALUE_PATTERN =
  /(?:[+-]?\d+:\d{2}\.\d{3}|[+-]?\d+(?:\.\d+)?\s?(?:s\/lap|km\/h|MJ\/lap|kg\/lap|kg|laps?|pts|pos|%|°C)|\b\d+\/\d+\s+(?:laps?|valid)\b|\b\d+\s+(?:green\s+laps?|valid|invalid)\b|\b\d+(?:st|nd|rd|th)(?:\s+of\s+\d+)?\b|\b\d+\s+of\s+\d+\b|\bP\d+\b|\bS[1-3]\b)/g;

function highlightDetailValues(text: string): ReactNode[] {
  ...
  for (const match of text.matchAll(DETAIL_VALUE_PATTERN)) {
    ...
    parts.push(
      <span key={`${value}-${index}`} className="font-medium text-zinc-200 tabular-nums">
        {value}
      </span>,
    );
